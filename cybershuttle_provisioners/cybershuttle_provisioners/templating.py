...
class ScriptTemplate:
    ...
    def get_all(self) -> list[Path]:
        return sorted(self.template_directory.glob("*.sh"))

    def choose_template(self) -> Path | None:
        ...
        if len(script_templates) >= 1:
            ...
            while True:
                template_to_install = input(
                    "Please choose a template to install using the \033[94midentifier\033[0m:\033[94m "
                )
                ...
                try:
                    ...
                    print(f"{Color.F_LightRed}template with id {template_to_install} not found!{Color.F_Default}")
                    continue
    ...
    def use(self, loginnode=None, user=None, proxyjump=None, dry_run=False):
        ...
        if self.template:
            self.template = self.template_directory / self.template.with_suffix(".sh")
            script_to_install = self.template
        else:
            script_to_install = self.choose_template()
        assert script_to_install is not None
        ...
        for type_spec, value in set_kernel_specs.items():
            for replace_item, replace_value in var_values.items():
                if "$" + str(replace_item) in value:
                    set_kernel_specs[str(type_spec)] = set_kernel_specs[str(type_spec)].replace(
                        "$" + str(replace_item), replace_value
                    )
        ...
        if not "displayname" in set_kernel_specs.keys():
            while True:
                kernel_displayname = input(
                    f"Display Name of the new Jupyter kernel (will be shown in e.g. JupyterLab): "
                )
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"{Color.F_LightRed}Invalid format for Slurm parameters! Correct format would be -> account=hpcgroup1,time=01:30:00,reservation=MyRes{Color.F_Default}"
                )
                continue
    ...
    def edit(self, editor: str | None = None):
        ...
        if editor is None:
            try:
                ...
                raise NoEditorGiven(
                    f"\033[91m$EDITOR not set. Please explicity set an editor with --editor (e.g. --editor vim)\033[0m\n"
                )
    ...
    @staticmethod
    def list_templates():
        script_templates = sorted(ScriptTemplate.template_directory.glob("*.sh"))
        if len(script_templates) >= 1:
    ...
    def save(self):
        if self.template:
            if self.template.suffix != ".sh":
                raise WrongFileType('Wrong filetype! ".sh" needed')
            file_destination = self.template_directory / self.template.name
            if file_destination.exists() and file_destination.is_file():
                raise FileAlreadyExists(f"File {file_destination} already exists!")
            copyfile(self.template, file_destination)
            return True
        else:
            raise OSError("No template given")
